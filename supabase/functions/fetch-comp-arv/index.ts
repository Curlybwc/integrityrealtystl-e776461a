...
serve(async (req) => {
  ...
  try {
    // Auth: require a signed-in investor or admin
    ...
    if (!compsResp.ok) {
      ...
      console.error("comps fetch failed:", t);
      return new Response(JSON.stringify({ error: "Comps fetch failed" }), {
